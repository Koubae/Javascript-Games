// @credit: https://codepen.io/ge1doot/details/LZdOwj
...
const theLastExperience = noWorkers => {
  ...
  const initRobots = () => {
    // ---- instanciate robots ----
    ground = canvas.height > 500 ? 0.85 : 1.0;
    const TOTAL_ROBOTS = 3;
    for (let i = 0; i < TOTAL_ROBOTS; i++) {
      let sizeMultiplier = i && i % 2 === 0 ? 2 : 0;
      dancers.push(
        new Robot(
          i * 360 / 7,
          80,
          Math.sqrt(Math.min(canvas.width, canvas.height)) / 6 + sizeMultiplier,
          (i + 2) * canvas.width / 9,
